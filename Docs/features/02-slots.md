...
> Currently, saving a script to a Slot just copies the raw string into the `hs_slots` array. It does **not** create a live reference or link to a Library item. If a user updates a script in the Editor and saves it to the Library, the Slot will **not** update. Agents must treat Slots as isolated clipboards until the synchronization roadmap item is built.
...
- **Slots vs. Library Tangle** — The UX and data flow between Slots and Library act as isolated clipboards rather than a unified system. (Resolution planned via synchronization task in `ROADMAP.md`).
